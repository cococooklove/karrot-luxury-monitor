...
- 엑셀 열은 `키워드 / 최소가격 / 최대가격 / 제외 / 끌올일수`. **키워드만 필수.**
  다이얼로그의 '샘플 엑셀 저장'이 이 머리글로 파일을 만들어 준다.
- 옛 7열 시트(`대분류 / 키워드 / 추가키워드 / 제외키워드 / 최소금액 / 최대금액 / 끌올일수`)도
  그대로 읽힌다 — 추가키워드는 키워드에 합쳐지고 대분류는 무시된다.
...
- **지역은 엑셀 열이 아니다.** '고급' 패널의 지역 트리에서 고른다(미선택 시 기본 166동,
  '전국 훑기' 체크 시 동단위 전국).
- 끌올일수는 검색 스윕에만 걸린다 — 앱 알림 payload 에는 끌올 시각이 없다.
