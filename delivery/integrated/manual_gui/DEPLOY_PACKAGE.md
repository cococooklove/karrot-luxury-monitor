...
- 엑셀 열은 `대분류 / 키워드 / 추가키워드 / 제외키워드 / 최소금액 / 최대금액 / 끌올일수` 7개.
  다이얼로그의 '샘플 엑셀 저장'이 이 머리글로 파일을 만들어 준다.
- **지역은 엑셀 열이 아니다.** 같은 '고급' 패널의 지역 트리에서 고른다(미선택 시 기본 166동,
  '전국 훑기' 체크 시 동단위 전국).
- 명품 브랜드 사전은 `daangn_ext`/`parse_luxury` 에 내장 — 키워드만 지정하면 필터됨

주의: '수동 검색' 탭의 '엑셀 크롤링'은 **다른 형식**을 쓴다(`지역 / 키워드 / 최소가격 / 최대가격`).
'엑셀 조건'용 샘플 파일을 그쪽에 넣으면 열 이름이 달라 거부된다. 두 파일을 섞지 말 것.
