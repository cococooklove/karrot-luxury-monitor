...
try {
  ...
  $act = New-ScheduledTaskAction -Execute $pyw -Argument "main.py --watchdog --watch" -WorkingDirectory $app
  ...
  Log ("  task karrotgui: pythonw main.py --watchdog --watch  (작업 폴더 " + $app + ")")
} catch {
...
Write-Host "[install] desktop icons: '당근 수동 검색' (--manual) / '당근 매물 감시' (--watch)" -ForegroundColor Green
