...
# 브랜드를 첫 줄에만 적고 아래를 비워 두는(또는 셀 병합) 시트가 흔하다.
# 그 빈 칸을 제품명 첫 어절로 짐작하면 '트위스트'가 브랜드로 등록되고,
# 그 줄은 '트위스트'만으로 매칭돼 남의 브랜드 매물까지 잡는다.
_fd = [("브랜드", "제품명", "최소가격", "최대가격", "제외"),
       ("루이비통", "슬렌더", 100000, 500000, ""),
       ("", "트위스트", 1000000, 3000000, ""),
       (None, "오거나이저", 200000, "", ""),
       ("보테가베네타", "카세트백", 1000000, 2500000, ""),
       ("", "아르코", 1500000, "", "")]
_fr, _fe = parse_rule_rows(_fd)
ck("브랜드 칸이 비면 바로 위 브랜드를 이어받는다",
   [r.brand for r in _fr]
   == ["루이비통", "루이비통", "루이비통", "보테가베네타", "보테가베네타"]
   and not _fe, f"{[r.brand for r in _fr]} / {_fe}")
ck("이어받은 브랜드로 매칭 키워드도 만든다",
   _fr[1].keyword == "루이비통 트위스트" and _fr[4].keyword == "보테가베네타 아르코",
   str([r.keyword for r in _fr]))
ck("등록 브랜드에 제품명이 섞이지 않는다",
   brands(_fr) == ["루이비통", "보테가베네타"], str(brands(_fr)))
ck("이어받으면 남의 브랜드 매물은 안 잡는다",
   RuleTable(_fr).verdict("샤넬 트위스트 락 백", 2000000)[0] == CUT
   and RuleTable(_fr).verdict("루이비통 트위스트 MM", 2000000)[0] == HIT)
ck("맨 위 줄부터 브랜드가 비면 그 줄은 건너뛰고 말해 준다",
   len(parse_rule_rows([("브랜드", "제품명"), ("", "트위스트"), ("루이비통", "슬렌더")])[0]) == 1
   and "브랜드" in parse_rule_rows([("브랜드", "제품명"), ("", "트위스트")])[1][0],
   str(parse_rule_rows([("브랜드", "제품명"), ("", "트위스트")])))
